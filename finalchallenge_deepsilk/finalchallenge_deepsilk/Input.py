...
class SetPointPublisher(Node):
    def __init__(self):
        ...
        # Declarar parámetros con descripciones
        self.declare_parameter('speed_rpm', 0.0)
        self.declare_parameter('signal_type', 'sine')
        self.declare_parameter('frequency', 1.0)

        self.timer_period = 0.0002439 # Período en segundos

        # Crear un publicador en el tópico /set_point
        self.signal_publisher = self.create_publisher(Float32, 'set_point', 100)
        self.timer = self.create_timer(self.timer_period, self.timer_cb)

        # Crear un suscriptor al tópico /motor_output
